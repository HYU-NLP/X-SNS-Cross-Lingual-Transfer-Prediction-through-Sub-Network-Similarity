...
def test(args, model, test_dataset, preprocessed_test_dataset,  lang):
    ...
    return exact_match
def shotdata(dataset,shot):
    shot_data = []
    avoid_overlapping = []
    while True:
        if len(shot_data)>= shot:
            break
...
def main():
    ...
    parser.add_argument('--lang', type=str, default='engilsh')
    ...
    # model.load_state_dict(torch.load(f'/home/tjay/Parameter-efficient/ProjectCode/output/modelpath/qa/{args.lang}.pt'))
    global tokenizer
    ...
    language = []
    for k in traindata:
        traindata[k] = Dataset.from_list(traindata[k][:128])
        language.append(k)
    ...
    for k in valdata:
        ...
        language2.append(k)
    val ={}
    source =['english'	,'arabic',	'bengali',	'finnish',	'indonesian','korean',	'russian',	'swahili','telugu']
    target =['english'	,'arabic',	'bengali',	'finnish',	'indonesian','korean',	'russian',	'swahili','telugu']
    for t in target:
        val[t] = make_preprocessed_test_dataset(
                valdata[t], preprocess_validation_examples
        )
    ...
    df = pd.DataFrame(index=source,columns=target)
    for s in source:
        for t in target:
            model = AutoModelForQuestionAnswering.from_pretrained(args.model).to(device)
            model.load_state_dict(torch.load(f'/home/tjay/Parameter-efficient/ProjectCode/output/modelpath/qa/{s}.pt'))
            tokenizer = AutoTokenizer.from_pretrained(args.model)
            tdata = shotdata(traindata[t],128)
            train_dataloader = make_dataloader(tdata, preprocess_training_examples, args.batch_size)
            for i in range(10):
                train(args, model, train_dataloader)
            val_em = test(args, model, valdata, val[t],t)
            df.loc[s,t] =val_em
    ...
    print('Done!')
    df.to_csv('/home/tjay/Parameter-efficient/ProjectCode/output/output_csv/fewshot_qa.csv')
    # save model
    #torch.save(model.state_dict(), f'/home/tjay/Parameter-efficient/ProjectCode/output/modelpath/qa/{args.lang}.pt')
